...
//static void promoteAllActive(void);
#endif  //CS333_P4
...
yield(void)
{
  ...
  if(curproc->budget <= 0)//budget is used, need to demote
  {
    if(curproc->priority == 0)
    //already in lowest prio, reset budget
    {
...
sleep(void *chan, struct spinlock *lk)
{
  ...
  if(p->budget <= 0)//budget is used, need to demote
  {
    if(p->priority == 0)
    //already in lowest prio, reset budget
    {
...
promoteAllActive(void)
{
  ...
  for(int i = (MAXPRIO-1); i >= 0; --i)
  {
    ...
    while(p)
    {
      if(stateListRemove(&ptable.ready[p->priority], p) == -1)
      {
        panic("statelistremove in promoteAllReady");
      }
      assertPriority(p, p->priority, __FUNCTION__, __LINE__);
      p->priority += 1;
      p->budget = DEFAULT_BUDGET;
      stateListAdd(&ptable.ready[p->priority], p);
    }
